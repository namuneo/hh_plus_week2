...
# 멘토링 요약
        ...
        - erd 개선 및 반영(필수는 아님)
        - inmemoryRepositoryImpl 을 jpaRepositoryImpl로 전환하는 형태로 << jpa레포지토리를 활용하여 테스트 통과가 중요함!!!
        
        심화과제
        - 인기 상품 조회 등에 어떠한 인덱스를 만들어야 할지 고민
	        - 데이터를 적재한 후에 쿼리를 날려 보아야 한다
	        - 동일한 dataset을 넣으면 적절한 쿼리를 측정할 수 없다.
	        - 운영되는 서비스에 유사한 패턴으로 데이터를 적재해야 한다
	            - 주문 완료 상태의 데이터와(구매확정), 주문요청 데이터가 있을 것...
	            - 주문완료 상태의 데이터가 더 많음
	                - 이 부분을 고려해서 데이터를 적재한 후 쿼리를 조회
	                - 배치 or k6같은 성능 테스트 툴을 사용
	                - procedure나 function을 사용해서 데이터 적재도 괜찮
	                - AI를 이용해서 데이터 넣어도 괜찮
        - 하나의 PR로 처리해도 괜찮
